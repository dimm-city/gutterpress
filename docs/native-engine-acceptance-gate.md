...
3. **`citizen-file:right` named-page margin-box content fails to paint** on the field guide's first `citizen-file` page (p7) — root-caused to a narrow Chrome 151 native-print defect specific to the first page of a named-page transition; `:left` and later `:right` continuations of the SAME named page render correctly. No fix landed; a `:right`/`:left`-pseudo-only (unnamed) workaround is proven reliable and recommended pending upstream Chromium investigation.
