...
| 08-08 | C.15 view modes (single/two-up) — IMPLEMENTED | scroll-snap-based paging: single mode snaps every page, two-up snaps recto+verso pairs together; content never moves (sheets stay exactly where `decorate.ts` painted them) | works (DOM repositioning) | = | live drive + screenshot mid-run (p20/p21 of the 34pp book, not p1): `goToPage(20)` under `view-two-column` settles with both full sheets visible side by side, content intact — `/tmp/two-up-mid.png`. `pageStep()` already made next/prevPage step by 1 or 2; this adds the matching manual-scroll (wheel/trackpad) behavior via CSS `scroll-snap-type`/`scroll-snap-align`, not a DOM-repositioning approach (that shipped broken once already — see the retired `decoration.setSpread()` history in `viewer.css`) |
...
- **View modes (C.15)** — implemented as CSS scroll-snap (not DOM repositioning, which shipped broken once already). Single mode snaps every page; two-up snaps recto+verso pairs. Verified live with a mid-run screenshot.
