...
| `packages/cli/tests/compat/preview-smoke.pw.ts` | Playwright test: renders user guide + design guide + feature probe in chromium/firefox/webkit; asserts render completes, page count > 0 and equals the event's `totalPages`, zero collapsed content pages, no uncaught errors, page count within ±20% of the Chromium baseline. **Status: 3/3 passing locally.** |
