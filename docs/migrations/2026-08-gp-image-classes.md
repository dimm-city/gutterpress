...
- **Pin**: `.gp-pin` (+ `.gp-top`/`.gp-bottom`/`.gp-left`/`.gp-right`)
  pins an image within its `@page`/`@spread` container. Later in the same
  release, `.gp-flush` was added to that family: with `.gp-pin` and an edge,
  it takes the art past the page's margin to the paper edge (inert on its
  own).
