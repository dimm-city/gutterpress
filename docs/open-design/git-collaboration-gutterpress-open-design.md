...
If a book has no explicit `source.files`, Print-MD renders every top-level `.md` file in that book alphabetically. Discovery is top-level only, so a `chapters/` directory always needs an explicit list.
...
The two never mix, and neither is a publication asset. A Print-MD plugin is a
...
Print-MD copies no stylesheets. A `styles:` entry is a path it **reads**, and
