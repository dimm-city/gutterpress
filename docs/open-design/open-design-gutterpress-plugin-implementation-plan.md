...
`<question-form id="gutterpress-brief">` containing only unresolved fields and
...
od plugin pack ./plugin --out /tmp/gutterpress-publishing-0.2.0.tgz --json
...
```bash
od plugin install gutterpress-publishing
```
