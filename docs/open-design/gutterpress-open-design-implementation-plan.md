...
Gutterpress recognizes, in order:
...
`resolveActiveStyles` is the single source of stylesheet truth: the renderer, the CLI's lint runner, and the desktop app's CSS editor all call it, so what is edited is what is rendered.
...
The CLI already prints the bound preview URL (`Preview server running at http://localhost:<port>`), and the preview server answers `GET /api/status` with `{ hasInput, currentPath }` for liveness and project identification. The desktop app may add a generic **Copy preview URL** action.
...
- a Gutterpress-created `.gutterpress` or `.design` project directory;
- required token JSON;
