...
```markdown
@section .two-column .dc-citizen-file
...
Authors write `@section .two-column .dc-citizen-file` once and **every element inside it** picks up form-styling. The same section component dropped into any other chapter/page/book produces the same character-sheet look — no setup required.
