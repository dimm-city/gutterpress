...
| `packages/desktop` entry: `tests/**/*.{ts,js,mjs}` | Yes. | Removed alone: gated exit 1 — `Unused files (16)`, naming all 8 `tests/integration/*.pw.mjs`, `tests/perf/bench/novel-50p/generate.mjs`, and the 7 `tests/updater/*.test.ts`. | **Load-bearing** (same as the `src/**/*.type-test.ts` row below: nothing else roots these test files). |
