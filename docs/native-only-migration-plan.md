...
**`--format html`** — ship the **continuous, self-contained `book.html` plus the viewer bundle**, and let it fragment in the browser on load. Reason: `book.html` is *already* a complete self-contained artifact (`asset-inline.ts` inlines all CSS and fonts; images are document-relative), so publishing is one file copy plus one `<script>` tag — no headless Chromium at build time, no DOM serialization, and the page still reads with JS off.
...
- **Narrow viewports.** A fixed 6×9in sheet is unreadable on a phone. `mount()` already reads `location.search` for `designer`; add a viewport check plus a `?view=scroll` param — if the window is narrower than the sheet, don't mount and leave the continuous document. ~5 lines in `viewer/index.ts`, plus a toggle if wanted.
