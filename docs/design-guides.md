...
`gutterpress` discovers the manifest from whichever directory you point at,
so `gutterpress build ./design-guide` and `gutterpress build .` (the book) both
...
A `styles:` entry is a path Gutterpress *reads*, not a file it ships — the
...
```sh
gutterpress preview ./design-guide
```
...
3. Push to `main`. The workflow runs `gutterpress build ./design-guide
...
- **Netlify / Cloudflare Pages** — set the publish directory to the
  output dir, and the build command to `bunx gutterpress build ./design-guide
  --format html --out <publish-dir>`.
- **S3 / object storage** — sync the output dir to a bucket with static
  hosting enabled.

The desktop output uses relative paths for its scripts and styles, so subpath
...
```sh
gutterpress build ./design-guide --format pdfx --icc path/to/profile.icc
```

## Side-manifest variant

If you'd rather keep the guide content in the same directory as the book
and just have a second manifest, every Gutterpress command takes
`--manifest`:

```sh
gutterpress build . --manifest ./design-guide.manifest.yaml --format html --out ./_site
```
...
- [Gutterpress User Guide](../examples/gutterpress-user-guide/) — markdown directives,
  page control, CSS theming, and all core features.
- [User Guide: Chapter 4 — Styling & Theming](../examples/gutterpress-user-guide/04-styling-theming.md) — theme examples and
  CSS conventions.
