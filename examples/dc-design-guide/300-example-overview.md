...
**Annotation paragraph** — a short paragraph naming the active page template, the CSS classes applied to `@page` or `--- {page ...}`, and any macro directives (`@specialty`, `@learning-path`, `@skill`, `@chapter-opener`). Cross-references point to the spec chapters for token details.
