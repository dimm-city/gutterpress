...
**Do not do this.** No `ttrpg` package exists anywhere Gutterpress looks — not in the bundled plugin registry, not on npm as something this project publishes. The build fails immediately, and the failure message tells you to install an arbitrary, unrelated package:
...
Do not run `bun add ttrpg`. There is nothing at that package name that this project ships or endorses. The rest of this chapter shows what Gutterpress actually provides for TTRPG content — all of it buildable right now, with no plugin and no install step.

## Start From the TTRPG Template

`gutterpress new` includes a `ttrpg` starter template that scaffolds a project with the bundled `ttrpg-supplement` theme already wired into the manifest:

```bash
gutterpress new "My Supplement" --template ttrpg
```
...
A stat block is a table. Gutterpress's tables already page-break intelligently and repeat headers, which is what a stat block needs:
...
Boxed, italicized text a game master reads to players is a blockquote. Gutterpress's default blockquote styling — left border, tinted background, italic — already reads as a read-aloud box with zero configuration:
...
There is no `@[...]` reference syntax. Use a standard markdown link to a heading's explicit `{#id}` — headings do **not** get an automatic id from their text in Gutterpress, so give any heading you plan to link to an explicit one:
...
A full creature entry combining everything above — a linkable heading, read-aloud text, a color-coded stat block, and dice notation — built entirely from core Gutterpress features. Text earlier in a chapter can link forward to it:
...
These are TTRPG conventions built from generic Gutterpress primitives, not a game-specific mode you turn on. For non-game books, skip the stat-block and read-aloud styling and write standard prose — see Chapter 2, *Writing Your Content*.
