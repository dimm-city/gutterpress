...
It prints the Gutterpress version, your OS/architecture, the CLI config
...
> **Windows note:** Gutterpress uses `where` instead of `which` to probe for tools. If you have `busybox` or other POSIX emulators installed, ensure `where` is the one on `PATH`.
