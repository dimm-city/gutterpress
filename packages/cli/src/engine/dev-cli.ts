 ...
 *   bun src/engine/dev-cli.ts export book.html -o dist/         self-contained viewer bundle (iframe embed)
 *
 * No bundler at runtime: the dev server is `node:http` + `ws`, matching the
 * host repo's architectural rule §1. The viewer bundle it serves is the same
 * prebuilt/embedded `gutterpress-viewer.js` the CLI package ships (`../assets/engine/`,
 * generated by `scripts/build-engine-bundles.mjs`) — read directly from disk
 * here since this tool only ever runs from a source checkout.
 */
...
const VIEWER_JS = join(import.meta.dir, "..", "assets", "engine", "gutterpress-viewer.js");
...
function cmdExport(positional: string[], flags: Record<string, any>) {
  ...
  writeFileSync(join(outDir, "gutterpress-viewer.js"), readFileSync(VIEWER_JS));
