 ...
 * conf-20).
 *
 * The CLI (packages/cli/src/assets/vendor/paged.polyfill.js) and the desktop
 * (packages/desktop/static/vendor/paged.polyfill.js) ship byte-identical copies
 ...
 * desktop. This test fails the moment the two diverge.
 *
...
const DESKTOP_COPY = path.join(
  import.meta.dir,
  "../../../../desktop/static/vendor/paged.polyfill.js",
);
...
describe("vendored paged.polyfill.js copies", () => {
  it("are byte-identical across packages/cli and packages/desktop", () => {
    const cli = sha256(CLI_COPY);
    const desktop = sha256(DESKTOP_COPY);
