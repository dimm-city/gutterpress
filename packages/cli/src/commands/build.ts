...
export default defineCommand({
  ...
  args: {
    ...
    format: {
      type: "string",
      description: "Output format: html | pdf | pdfx (default: pdf)",
    },
    out: {
      type: "string",
      description:
        "Output directory. For --format pdf|pdfx, --out may also be a .pdf file path.",
    },
    title: {
      type: "string",
    ...
    "pdfx-flavor": {
      type: "string",
      description:
        "PDF/X flavor (x1a or x3). --format pdfx only. Defaults to manifest pdfx.flavor (preset: x1a).",
    },
    icc: {
      type: "string",
    ...
    "strip-annotations": {
      type: "boolean",
      description:
        "Strip PDF annotations for PDF/X compliance (default: true with --format pdfx)",
    },
    "skip-lint": {
      type: "boolean",
      description: "Skip CSS linting (default: lint runs for pdf/pdfx)",
    },
    "skip-pre-validate": {
      type: "boolean",
      description:
        "Skip pre-build validation (default: runs for pdf/pdfx)",
    },
    "skip-post-validate": {
      type: "boolean",
      description:
        "Skip post-build PDF/X validation (default: runs for pdfx)",
    },
  },
  async run({ args }) {
