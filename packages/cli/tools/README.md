...
**When to use:** You want a hand-tuned or simplified wrap polygon instead of the image's raw alpha silhouette. For the common case, authors should just use the core `.gp-shape` class — `shape-outside: url(...)` prints as a compact vector PDF with fully extractable text (measured 2026-08-11, Chromium 141: ~13 KB for a shaped page; `marker-css-image-shape.test.ts` guards this). Reach for a polygon only when the alpha silhouette itself is unsatisfying (noisy alpha, deliberate looser wrap).
