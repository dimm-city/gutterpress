/**
 * Static guard: every runtime import in packages/lib/src/ MUST be declared in
 * packages/lib/package.json#dependencies (not devDependencies).
 *
  ...
  it("every runtime import in lib/src must be a dependency (not a devDependency)", () => {
    ...
    if (violations.length > 0) {
      const report = violations
        .map((v) => `  • ${v.package}\n    at ${v.where}\n    ${v.reason}`)
        .join("\n\n");
      const fix = `\n\nFix: move the offending packages from devDependencies to dependencies in packages/lib/package.json. If a package is genuinely optional/peer, add it to RUNTIME_OPT_OUT in ${relative(LIB_ROOT, __filename)} with a comment explaining why.`;
      throw new Error(
