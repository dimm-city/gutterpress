 ...
 *   - packages/lib/src/checks/tool-check.ts — the validation gate, decides
 *     which checks to skip when their backing CLI tool isn't installed.
 *   - packages/lib/src/lib/chromium.ts — PATH-fallback for Chromium when the
 *     fixed-path scan misses non-default installs (Scoop, Chocolatey, Brave,
 *     Vivaldi, Arc, portable installs).
 *   - packages/lib/src/lib/build-runner.ts — pre-flight check at the top of
