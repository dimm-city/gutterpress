---
name: gutterpress-publishing
description: Use this plugin when the user wants to design or refine an existing Gutterpress paged publication without replacing its renderer or editing generated output.
triggers:
  - gutterpress
  - paged publication
  - book theme
...
You are editing an existing Gutterpress publication in place. Gutterpress remains the
renderer. The running Gutterpress preview is the visual and pagination authority.
...
- **Preview URL:** prefer an active loopback Gutterpress Browser tab, otherwise try
...
ambiguous, emit exactly one `<question-form id="gutterpress-brief">` containing
...
`[form answers — gutterpress-brief]` and override inferred defaults.

Use this shape, deleting questions that are already resolved:

```html
<question-form id="gutterpress-brief" title="Confirm publication scope">
{
  "description": "Only the unresolved decisions are shown. No files will change until you submit this brief.",
  "questions": [
    { "id": "bookPath", "label": "Which Gutterpress book should change?", "type": "text", "required": true },
    ...
    { "id": "previewUrl", "label": "What is the running Gutterpress preview URL?", "type": "url", "required": false }
  ]
}
</question-form>
```
...
- Do not add `source.assets` or `output` to a manifest — both were removed from Gutterpress and now fail the build outright.
...
- Do not run Git, package managers, or start/stop the Gutterpress preview. Ask the
  user to run Gutterpress commands when they are needed.
...
1. Resolve the selected book beneath the imported Open Design project root.
    2. Find the first existing manifest: `manifest.yaml`.
...
Every `styles` entry names a real file at that exact path. Gutterpress reads and
...
`css/main.css`; Gutterpress then falls back to the first discovered CSS file. Do
...
6. Authored Gutterpress plugin change only when the behavior cannot be expressed in CSS or semantic Markdown.
...
- Inspect the resolved Gutterpress preview when it is available. An active Browser
...
- After typography, page geometry, columns, spacing, font, image-size, or
  ...
  Gutterpress rebuilds and full-reloads the complete document after every watched
