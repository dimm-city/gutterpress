...
## Patch 7 - `renderEpoch`: retire every block view without remounting

**Why.** The Patch 1 seam is consulted only when a block VIEW is built, and
a view is reused for as long as its view data is unchanged (`canReuse`).
Gutterpress answers the seam from a projection of the document that is
rebuilt asynchronously after an edit; a block built while that projection
was stale falls through to the upstream rendering (a marker line shows as
text, a plugin's region as the markdown it was written from) and stayed
that way until something else happened to rebuild it. Remounting the view
would rebuild every block, but it also throws away the model's edit
history, which the controller owns, and the caret with it.

**What.** A render epoch the view owns, threaded to every block view
through the options it is built with. `gpRerender()` bumps it; on the next
render every block view's `canReuse` sees a different epoch and rebuilds,
consulting the seam again, while the model, its selection and the
controller's history are untouched.

### Hunk 22 - `class T` constructor (`dist/index.js`)

Records `this._gpRenderEpoch = t?.gpRenderEpoch` alongside `this.data`.

### Hunk 23 - `T.canReuse` (`dist/index.js`)

`return this.data === e` becomes
`return this.data === e && this._gpRenderEpoch === t?.gpRenderEpoch`.
Subclasses that override `canReuse` call `super.canReuse`, so the code
block view's own factory-version check composes with this one unchanged.

### Hunk 24 - `EditorView._gpRenderEpoch` (`dist/index.js`)

A settable observable, initially `0`, declared next to
`_embeddedCodeEditorFactoryVersion`.

### Hunk 25 - `EditorView.gpRerender()` (`dist/index.js`)

Bumps the observable, exactly the way `refreshEmbeddedCodeEditors()` bumps
the factory version.

### Hunk 26 - the render autorun (`dist/index.js`)

Reads the epoch as a dependency and, when it is not `0`, spreads it into
the options handed to `DocumentViewNode.create` as `gpRenderEpoch`. While
it is `0` the options object is passed through untouched, so a host that
never calls `gpRerender()` runs the upstream path byte-for-byte.

### `dist/index.d.ts`

`EditorViewOptions.gpRenderEpoch?: number` (documented as set by the view,
never by hosts) and `EditorView.gpRerender(): void`.
