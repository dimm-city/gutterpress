...
beforeEach(async () => {
  root = await mkdtemp(path.join(os.tmpdir(), "print-md-file-launch-"));
});
...
test("argv extraction accepts absolute/relative .md paths and ignores other arguments", () => {
  ...
  expect(
    markdownFilePathsFromArgv(
      ["viewer", "--flag", "chapter.md", absolute, `"${absolute}"`, "notes.markdown", "https://example.test/x.md"],
      root,
    ),
...
test("a chapter resolves to the nearest manifest-bearing ancestor", async () => {
  ...
  await writeFile(path.join(nestedBook, "manifest.yml"), "title: Nested\n");
...
test("a chapter resolves through the persisted print-md.yaml legacy filename", async () => {
  const project = path.join(root, "legacy-project");
  const chapter = path.join(project, "chapters", "one.md");
  await mkdir(path.dirname(chapter), { recursive: true });
  await writeFile(path.join(project, "print-md.yaml"), "title: Legacy\n");
  await writeFile(chapter, "# One\n");
...
test("an unrelated Markdown file is rejected instead of opening a loose folder", async () => {
  ...
  if (result.type === "error") {
    expect(result.message).toContain("isn't inside a print-md project");
