...
const swSource = readFileSync(join(viewerRoot, "src", "service-worker.ts"), "utf8");
...
test("service worker precaches the vendored paged.js (offline preview, #33)", () => {
  ...
  expect(existsSync(join(viewerRoot, "static", "vendor", "paged.polyfill.js"))).toBe(true);
});
...
test("service worker uses a versioned cache and prunes old ones on activate (#33)", () => {
  ...
  expect(swSource).toMatch(/print-md-cache-\$\{version\}/);
