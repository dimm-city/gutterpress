...
function locateExe() {
  ...
  if (p === "win32") {
    const exe = join(distDir, "win-unpacked", "print-md-viewer.exe");
    if (!existsSync(exe)) fail("no win-unpacked/print-md-viewer.exe — run `npm run dist:win` first");
    return exe;
  }
  if (p === "darwin") {
    ...
    return join(appRoot, app, "Contents", "MacOS", "print-md-viewer");
  }
