...
export const POST: RequestHandler = defineRoute<{ dir: string; name: string }>({
  validate: (raw) => {
    ...
    const dir = requireWithinProjectRoot(requireAbsolute(body.dir, 'fs:createFolder'), 'fs:createFolder');
    const name = requireSegment(body.name, 'fs:createFolder name');
    return { dir, name };
  },
  call: async ({ body }) => {
    const target = requireWithinProjectRoot(path.join(body.dir, body.name), 'fs:createFolder');
    try {
