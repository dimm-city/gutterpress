 ...
 *    layout (these are the rules that preview.js built via buildDesktopStyleSheet
 *    and injectDesktopStyles in the original same-origin architecture).
...
export function buildDesktopStyles(bg: string): string {
  return `
/* Injected by Gutterpress desktop — not in preview.css (Paged.js strips @media pagedjs-ignore) */

/* Zoom — set via --gutterpress-zoom CSS custom property (JS sets the number) */
html { --gutterpress-zoom: 1; }
.pagedjs_pages { zoom: var(--gutterpress-zoom) !important; }
...
/* Debug mode — injected by Gutterpress desktop; scoped under body.debug */
body.debug {
