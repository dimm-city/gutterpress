 ...
 * HTTP calls and `$lib/files/files-capability`'s dialog/shell IPC calls
 * (SFE-P5c1), and the `onSaved` / `onConnected` / `onPublished` callbacks
 * (the panel wires these to toasts).
