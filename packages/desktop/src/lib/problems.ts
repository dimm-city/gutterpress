 ...
 *
 * Relative import (not the `$lib` alias) for `ProblemEntry` (SFE-P5c3): this
 * module is reached from `electron/api/publish.ts` (`publishPreflight`
 * reuses `$lib/preflight.ts`'s pure result-shaping, which imports this file)
 * — `tsc -p electron/tsconfig.json` has no `$lib` alias configured (that's
 * SvelteKit's, generated by `svelte-kit sync` into a program this one
 * doesn't share), so an aliased specifier fails to resolve there even though
 * it resolves fine under the SPA's own tsconfig. Same landmine
 * `files-capability.ts`'s header documents for the same reason.
 */
import type { ProblemEntry } from "./platform/dtos";
