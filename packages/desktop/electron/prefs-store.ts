// ──────────────────────────────────────────────────────────────────────────
// Desktop prefs (gutterpress-prefs.json) — session/per-project state persisted
...
export function createPrefsStore(deps: PrefsStoreDeps): {
  ...
  writePrefs(p: DesktopPrefs): Promise<void>;
  ...
  function prefsPath(): string {
    return path.join(deps.getUserDataDir(), "gutterpress-prefs.json");
  }

  async function readPrefs(): Promise<DesktopPrefs> {
    let raw: string;
    try {
  ...
  async function writeNow(prefs: DesktopPrefs): Promise<void> {
  ...
  function writePrefs(prefs: DesktopPrefs): Promise<void> {
  ...
  function updatePrefs(
    mutate: (prefs: DesktopPrefs) => DesktopPrefs,
