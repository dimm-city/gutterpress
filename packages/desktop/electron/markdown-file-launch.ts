 ...
 * Gutterpress manifest, and queues the result until the renderer subscribes.
 */
...
function unavailable(filePath: string): MarkdownFileLaunchEvent {
  return {
    type: "error",
    filePath,
    message: `Gutterpress couldn't open "${path.basename(filePath)}" because the file isn't available. Check that it still exists and that you can read it.`,
  };
}
