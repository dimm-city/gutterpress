...
test("get() before register() returns null", () => {
  const bridge = createHostBridge<DemoHooks>("__gutterpressTestBridgeUnset__");
  expect(bridge.get()).toBeNull();
});

test("register() then get() returns the same object reference", () => {
  const bridge = createHostBridge<DemoHooks>("__gutterpressTestBridgeSet__");
...
test("two bridges with different keys do not share state", () => {
  const a = createHostBridge<DemoHooks>("__gutterpressTestBridgeA__");
  const b = createHostBridge<DemoHooks>("__gutterpressTestBridgeB__");
...
test("two bridges sharing the same key share the same slot", () => {
  const first = createHostBridge<DemoHooks>("__gutterpressTestBridgeShared__");
  const second = createHostBridge<DemoHooks>("__gutterpressTestBridgeShared__");
