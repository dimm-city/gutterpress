...
beforeEach(async () => {
  const base = await mkdtemp(path.join(tmpdir(), "gutterpress-fs-crud-"));
...
test("fs/delete: end-to-end against a REAL git repo — the deleted file's content is recoverable from history", async () => {
  // Real gutterpress (isomorphic-git) end-to-end, not a fake: turn
  ...
  await gutterpress.providerFor({ type: "local-folder", path: projectDir }).initVersionHistory({
    projectDir,
    initialMessage: "Initial snapshot",
  });
  const gitSource = await gutterpress.detectProjectSource(projectDir);
  ...
  const history = await gutterpress.providerFor(gitSource).listHistory(projectDir);
  ...
  await gutterpress.providerFor(gitSource).restore({ projectDir, id: history[0]!.id });
