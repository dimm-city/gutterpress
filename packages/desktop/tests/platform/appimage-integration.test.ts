...
beforeEach(async () => {
  tmpRoot = await mkdtemp(path.join(os.tmpdir(), "gutterpress-appimage-"));
  home = path.join(tmpRoot, "home");
  await mkdir(path.join(tmpRoot, "downloads"), { recursive: true });
  await mkdir(path.join(tmpRoot, "resources"), { recursive: true });
  await mkdir(home, { recursive: true });
  sourceAppImage = path.join(tmpRoot, "downloads", "gutterpress-0.8.3.AppImage");
...
describe("XDG_DATA_HOME resolution", () => {
  ...
  test("a custom XDG_DATA_HOME moves the desktop entry and icon but NOT the AppImage (~/.local/bin is not an XDG data dir)", () => {
    ...
    expect(paths.desktopEntry).toBe("/custom/data/applications/city.dimm.gutterpress.desktop");
    expect(paths.icon).toBe("/custom/data/icons/hicolor/512x512/apps/city.dimm.gutterpress.png");
    expect(paths.appImage).toBe("/home/w/.local/bin/gutterpress.AppImage");
  });

  test("the managed AppImage filename never collides with the CLI's ~/.local/bin/Gutterpress", () => {
    expect(path.basename(resolveAppImagePaths("/home/w").appImage)).toBe("gutterpress.AppImage");
  });
});
...
describe("desktop entry", () => {
  ...
  test("contains exactly the agreed fields", () => {
    expect(entry).toBe(
      [
        ...
        'Exec="/home/w/.local/bin/gutterpress.AppImage" %f',
        "TryExec=/home/w/.local/bin/gutterpress.AppImage",
  ...
  test("Exec quotes a path with spaces so it stays ONE argument", () => {
    expect(escapeExecArgument("/home/My Apps/gutterpress.AppImage")).toBe(
      '"/home/My Apps/gutterpress.AppImage"',
    );
  });
...
describe("unsupported environments", () => {
  const cases: Array<[string, Partial<AppImageEnv>, UnsupportedReason]> = [
    ...
    ["a relative APPIMAGE value", { appImagePath: "./desktop.AppImage" }, "not-appimage"],
  ];
