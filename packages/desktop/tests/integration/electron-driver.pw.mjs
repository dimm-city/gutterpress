#!/usr/bin/env node
/**
 * Drive the packaged Electron desktop end-to-end on the current platform.
 *
 * - Launches the gutterpress Electron app via Playwright's _electron.
...
// executablePath points at the packaged gutterpress.exe (or .AppImage)
...
// below sees the wrong screen).
 const userDataDir = mkdtempSync(join(tmpdir(), "gutterpress-etest-"));
...
try {
  ...
  // Look for any of the strings the desktop's +page.svelte renders.
  const sentinels = ["Open a folder", "Open Folder", "Gutterpress"];
  ...
  // The desktop loads the preview URL via iframe navigation. We verify the
