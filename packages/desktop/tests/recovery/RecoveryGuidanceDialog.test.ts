 ...
 * in packages/desktop/src/lib/components/RecoveryGuidanceDialog.svelte.
 *
 * Spec coverage:
 ...
 *  - PWA-clean: no value import of gutterpress in the component source
...
describe("RecoveryGuidanceDialog — module exists and is PWA-clean", () => {
  ...
  test("does NOT value-import gutterpress (PWA-clean §8)", async () => {
    const source = await readFile(COMPONENT_PATH, "utf-8");
    ...
    const valueImportPattern =
       /import\s+(?!type\s)(?:\{[^}]*\}|\*\s+as\s+\w+|\w+)\s+from\s+['"]@dimm-city\/gutterpress['"]/;
