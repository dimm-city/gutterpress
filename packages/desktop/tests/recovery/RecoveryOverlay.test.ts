 ...
 *
 * Coverage areas:
 * 1. PWA-cleanliness: the module must not value-import gutterpress
...
describe("PWA-cleanliness (CLAUDE.md §8 / ADR 0004)", () => {
  ...
  test("does not value-import gutterpress (import type only)", () => {
    ...
    const valueImportPattern =
      /import\s+(?!type\s)(?:\{[^}]*\}|[\w*]+)\s+from\s+["']@dimm-city\/gutterpress["']/;
...
test("this test file itself does not value-import gutterpress", () => {
